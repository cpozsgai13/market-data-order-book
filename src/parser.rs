...
pub fn load_messages(path: &Path) -> Result<Vec<CoreMessage>, std::io::Error> {
     load_messages_inner(path)
}

fn load_messages_inner(path: &Path) -> Result<Vec<CoreMessage>, std::io::Error> {
...
fn parse_line(line: &str) -> Option<CoreMessage> {
    // Split at the first space to get the action keyword.
    let (action_str, rest) = split_first(line)?;
    let action = parse_action(action_str);

    match action {
        Action::Symbol => parse_symbol(rest).map(CoreMessage::Symbol),
        Action::Buy    => parse_add_order(rest, Side::Bid).map(CoreMessage::AddOrder),
        Action::Sell   => parse_add_order(rest, Side::Ask).map(CoreMessage::AddOrder),
        Action::Modify => parse_modify(rest).map(CoreMessage::ModifyOrder),
...
fn parse_symbol(rest: &str) -> Option<SymbolMsg> {
    let mut iter = rest.splitn(3, ' ');
    let inst_id:   InstrumentId = iter.next()?.parse().ok()?;
    let price:     Price        = Price::from(iter.next()?.parse::<f64>().ok()?);
...
fn parse_add_order(rest: &str, side: Side) -> Option<AddOrderMsg> {
    let mut tokens = rest.split_whitespace();
    let order_type: OrderType   = parse_order_type(tokens.next()?)?;
    let inst_id:    InstrumentId = tokens.next()?.parse().ok()?;
    let price:      Price        = Price::from(tokens.next()?.parse::<f64>().ok()?);
...
fn parse_modify(rest: &str) -> Option<ModifyOrderMsg> {
    let mut tokens = rest.split_whitespace();
    let inst_id:  InstrumentId = tokens.next()?.parse().ok()?;
    let order_id: OrderId      = tokens.next()?.parse().ok()?;
